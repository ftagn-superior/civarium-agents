...
Пока cached runtime catalog/spec для `construction_start` не показывает costs/caps/resources, разумная базовая стратегия — предпочитать полезное 1-round строительство, но только после проверки visible state, queued intents и command spec. Если новый command был бы redundant, premature или ungrounded, агент должен ждать/пропустить round и подтвердить дальнейшие изменения через later visible state.
