...
The most important part of the loop is the decision window after observation and before submission. The agent should creatively judge the visible state and decide whether the best move is a command, waiting for a pending outcome, or deliberately skipping the current round.
...
6. Enter the creative decision window:
   - identify what changed since the last observation;
   - identify pending queued intents and unresolved outcomes;
   - compare available command candidates from the cached catalog;
   - decide whether to submit a command, wait, or skip this round.
7. When reasoning about possible actions, use the cached catalog list of available commands.
8. Before using a specific command, call `get_civarium_command_spec(command_type)` if that spec is not already cached or payload details are uncertain.
9. If no valid beneficial command exists, do not submit a filler command. Report the grounded reason and wait for the next useful observation.
10. Build the payload from the command payload schema.
11. Generate a fresh UUID for `client_command_id`.
12. Submit the command with `submit_command`.
13. If the receipt has `is_valid=false`, inspect `checks`, correct the intent, and submit a new intent with a new UUID.
14. If the receipt has `is_valid=true`, call `list_queued_submitted_commands(round_id)`.
15. Treat a queued command as admitted intent, not world change.
16. Call `wait_next_round(after_round_id=round_id)` when waiting is appropriate.
17. After the round changes, call `get_visible_state` again.
18. Update compact campaign notes if persistent memory is available and the update is durable.

## Command Decision Gate

Submit a command only when all of these are true:

- current `round_id` is fresh;
- visible state was read this turn;
- command type exists in the cached catalog;
- command spec is known enough to build the payload;
- payload matches the command spec;
- the intended benefit is grounded in visible state, strategy, or rule facts;
- no required assumption depends on hidden state;
- no unresolved queued intent makes the new command redundant or premature;
- a fresh `client_command_id` is available for this new intent.

If the gate fails, the agent should wait, observe, update memory, or report `blocked` instead of submitting a speculative command.
...
- Do not submit low-quality filler commands merely to act every round.
...
If waiting or skipping is the best move, report it as an intentional decision, not as indecision.
...
6. **Autopiloting commands.** The agent's job is to choose well, not merely to submit something every round.
...
- [ ] The command decision gate passed, or waiting/skipping was chosen with a grounded reason.
